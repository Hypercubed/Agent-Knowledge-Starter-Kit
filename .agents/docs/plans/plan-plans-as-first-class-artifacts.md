---
id: plan-plans-as-first-class-artifacts
title: "Plans as first-class artifacts"
last_updated: 2026-04-22
description: >
  Promote plans into a first-class, searchable, portable artifact type living
  under .agents/docs/plans/, with a canonical frontmatter schema, verified
  docs-search integration, lifecycle tooling, and a write-plan skill —
  superseding plan-rules-in-scaffold.
tags: [plans, docs-search, skills, workflow, schema, kit]
status: active
kind: meta
supersedes: plan-rules-in-scaffold
outcome_decisions:
  - dec-plans-live-under-docs-plans-not-agents-plans
...
- [Regenerate `example/` when the portable kit or bootstrap changes](../decisions/dec-regenerate-example-when-portable-kit-changes.md)
- [`docs-search` stays canonical for `.agents/` knowledge; host-native search is not the default fallback policy](../decisions/dec-docs-search-remains-canonical-over-host-native-search.md)
- [Maintainer plans live under `.agents/docs/plans/`, not `.agents/plans/`](../decisions/dec-plans-live-under-docs-plans-not-agents-plans.md)
...
- `plan-rules-in-scaffold` superseded by this plan
...
`plans/archive/` only. Every plan id **must** start with `plan-`; decisions
(`dec-`) and troubleshooting (`ts-`) use separate prefixes (see `MAINTENANCE.md`).
...
- `id` — stable slug matching filename without `.md`; lowercase `[a-z0-9_-]`;
  must start with `plan-`; unique within `plans/` and `plans/archive/`.
...
- `anticipated_decisions` — planned **`dec-`** decision ids not yet filed; a distillation
  queue. Replace with real ids once files exist.
- `outcome_decisions` — durable **`dec-`** / **`ts-`** entry ids created or updated because of this
  plan. Fill in near completion.
- `supersedes` / `superseded_by` — other **`plan-`** ids forming a chain.
- `related_plans` — parallel tracks or dependencies (**`plan-`** ids).
...
status: active
kind: initiative
anticipated_decisions: [dec-auth-token-rotation-policy]
consumer_portable: false
---
```

Then continue the file body with **`## Related decisions`** and Markdown links to `../decisions/<dec-…>.md` or `../troubleshooting/<ts-…>.md` (not a YAML `related_decisions` list).
...
|---|---|---|
| `plan-add-integrations.md` | `id`, `last_updated`, `description`, `tags` | inline `active` → frontmatter |
| `plan-add-knowledge-search.md` | `id`, `last_updated`, `description`, `tags` | `planned` → `draft` or `active` |
| `plan-add-script-tests.md` | `id`, `last_updated` | `draft` ✓ |
| `plan-agent-role-guidance.md` | `id`, `last_updated` | `planned` → `draft` |
| `plan-consumer-upgrade-path.md` | `id`, `last_updated` | `partially implemented` → `active` |
| `plan-integrate-docs-search-*.md` | `id`, `last_updated`, `description`, `tags` | `implemented` → `completed` |
| `plan-out-of-repo-trees.md` | `id`, `last_updated` | `draft` ✓ |
| `plan-repo-centric-wiki-tooling.md` | `id`, `last_updated` | `in-progress` → `active` |
| `plan-rules-in-scaffold.md` | `id`, `last_updated` | `draft` ✓ |
| `plan-ship-structure-check-script.md` | all (no frontmatter block) | — |
| `plan-wiki-system.md` | all (no frontmatter block) | `Draft` → `draft` |
| `plan-worked-lifecycle-example.md` | `id`, `last_updated` | `planned` → `draft` |
...
1. **Related decisions** — bullet links to `../decisions/<dec-…>.md` or `../troubleshooting/<ts-…>.md` (body only; do not use YAML `related_decisions`).
...
4. **Tie back** — when a new decision exists because of the initiative, add a
   ...
   [dec-plans-live-under-docs-plans-not-agents-plans](../decisions/dec-plans-live-under-docs-plans-not-agents-plans.md).
...
- **Plan-involved tasks:** include a non-empty `related_plans` array of plan
  `id` strings (path-agnostic; stable across archive moves):

```json
"related_plans": ["plan-plans-as-first-class-artifacts"]
```
...
4. Past session bundles keep their original `related_plans` ids unchanged.

## Linking decisions ↔ plans

- **Plan → decision:** **`## Related decisions`** in the plan body with links to
  `../decisions/dec-<slug>.md` or `../troubleshooting/ts-<slug>.md`; optional `outcome_decisions`
...
|---|---|---|
| **write-plan** | Scaffold `docs/plans/plan-<slug>.md`; check plan id collision; emit related entry **links** in the body | **First** |
...
| **lint hooks** | Extend `knowledge-lint` to warn on stale `active` plans and orphaned `anticipated_decisions` | Deferred |
...
| `knowledge-lint` | Path-hygiene patterns for `archive-plan` link sweep; metadata contract checks |
...
`troubleshooting/`, and within `plans/` (with `dec-` / `ts-` / `plan-` prefixes).
...
- File `dec-plans-live-under-docs-plans-not-agents-plans` as a decision record under `.agents/docs/decisions/`. *(Done.)*
...
- Check `id` uniqueness within `plans/` and `plans/archive/` (plan ids must start with `plan-`)
...
- Update `dec-regenerate-example-when-portable-kit-changes` if new paths are added
...
- [ ] At least one `consumer_portable: true` sample plan ships via
  `generate-example`
- [x] Decision record filed for `dec-plans-live-under-docs-plans-not-agents-plans`
...
| Output | Destination |
|---|---|
| Path placement decision | `.agents/docs/decisions/dec-plans-live-under-docs-plans-not-agents-plans.md` |
