...
- `plans-in-scaffold` superseded by this plan
...
**Collision rule:** plan `id` values must not collide with any `id` under
`decisions/` or `troubleshooting/` (shared global namespace per MAINTENANCE.md).
Prefix plan ids (e.g. `plan-`) if needed to avoid clashes.
...
- `id` — stable slug matching filename without `.md`; lowercase `[a-z0-9_-]`;
  globally unique across `decisions/`, `troubleshooting/`, and `plans/`.
...
- `supersedes` / `superseded_by` — other plan `id` values forming a chain.
- `related_plans` — parallel tracks or dependencies.
...
4. **Tie back** — when a new decision exists because of the initiative, add a
   ...
   [plans-live-under-docs-plans-not-agents-plans](../decisions/plans-live-under-docs-plans-not-agents-plans.md).
...
```json
"related_plans": ["plans-as-first-class-artifacts"]
```
...
Verify `id` values are unique across `decisions/`, `troubleshooting/`, and
`plans/`. Run `index-docs.py` and spot-check `search-docs.py` results.
...
- File `plans-live-under-docs-plans-not-agents-plans` as a decision record under `.agents/docs/decisions/`. *(Done.)*
...
- Update `regenerate-example` decision if new paths are added
...
- [ ] At least one `consumer_portable: true` sample plan ships via
  `generate-example`
- [x] Decision record filed for `plans-live-under-docs-plans-not-agents-plans`
