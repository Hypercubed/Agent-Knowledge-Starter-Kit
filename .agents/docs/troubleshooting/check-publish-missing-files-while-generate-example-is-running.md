---
id: check-publish-missing-files-while-generate-example-is-running
title: "`check-publish.sh` reports missing files while `generate-example` is running"
last_updated: 2026-04-22
description: >
  Validation can fail with transient missing-file errors when checks run while
  `generate-example` is actively rebuilding `example/`.
