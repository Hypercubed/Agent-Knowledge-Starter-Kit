...
Important: in this repository, `scaffold/` is the published starter kit. The default adoption path is still to copy everything under `scaffold/` into `.agents/` at the root of a project that adopts the kit. You can also install only the portable skills under `scaffold/skills/` and run each skill's **Skill initialization** once so the repo gains the folders and template files that skill needs without copying the full kit.
...
1. **Full kit:** copy `scaffold/` into your project as `.agents/`. **Skills only:** copy `scaffold/skills/<name>/` into `.agents/skills/<name>/` for each skill you want, then follow the **Skill initialization** section at the top of each installed `SKILL.md` once (see [INSTALL.md](INSTALL.md#skill-first-install-optional)).
...
Follow [INSTALL.md](INSTALL.md). Treat `scaffold/` as the source tree that becomes `.agents/` in the target repo for a full install, or copy only `scaffold/skills/` when doing a skill-first install and run each skill's initialization from its `SKILL.md`. If `.agents/` already exists, merge conservatively and preserve existing repo-specific knowledge.
...
2. Add missing starter-kit directories from `scaffold/`: `docs/`, `agents/`, and `sessions/`, **or** install the skills you need from `scaffold/skills/` and run each skill's **Skill initialization** so missing template files and session layout are created without overwriting existing content.
