...
Maintainer roadmaps and multi-step initiatives (markdown under this folder; [index.md](plans/index.md) lists them). Plan files are **plan-only** guidance: they do not change shipped kit behavior until implementation is requested. See [plans-as-first-class-artifacts.md](plans/plan-plans-as-first-class-artifacts.md) for the YAML contract and lifecycle.
