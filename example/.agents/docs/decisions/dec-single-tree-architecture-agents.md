---
id: dec-single-tree-architecture-agents
...
status: accepted
depends_on:
  - dec-maintainer-skills-mark-internal-in-frontmatter
  - dec-regenerate-example-when-portable-kit-changes
---
...
- Maintainer-only skills in `.agents/skills/` are flagged with **`metadata.internal: true`** (see [Maintainer-only skills use `metadata.internal: true`](dec-maintainer-skills-mark-internal-in-frontmatter.md)).
- The disposable **`example/`** tree is regenerated when the illustrated install should match recent kit changes (see [Regenerate `example/` when the portable kit or bootstrap changes](dec-regenerate-example-when-portable-kit-changes.md)).
