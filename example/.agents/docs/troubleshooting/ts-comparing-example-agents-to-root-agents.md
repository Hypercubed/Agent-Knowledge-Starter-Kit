---
id: ts-comparing-example-agents-to-root-agents
...
- Use **`example/.agents/`** only as a disposable illustration of a bootstrap install, not as a second canonical tree to keep aligned byte-for-byte. When the illustrated tree should match recent portable or bootstrap changes, regenerate **`example/`** (see [Regenerate `example/` when the portable kit or bootstrap changes](../decisions/dec-regenerate-example-when-portable-kit-changes.md)).
