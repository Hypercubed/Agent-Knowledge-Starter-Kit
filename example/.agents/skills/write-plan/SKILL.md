...
Create a **plan-only** markdown file under `.agents/docs/plans/` that matches the **plan frontmatter contract** shipped with this skill ([`CONTRACT.md`](references/CONTRACT.md)), without hand-copying YAML. The repo’s normative copy of the same rules lives under [Frontmatter contract (plans) in `MAINTENANCE.md`](../../docs/MAINTENANCE.md#frontmatter-contract-plans); narrative and lifecycle context: [plans-as-first-class-artifacts](../../docs/plans/archive/plans-as-first-class-artifacts.md). If `CONTRACT.md` ever disagrees with `MAINTENANCE.md`, treat **`MAINTENANCE.md` as authoritative** until the skill contract is updated. All portable skill contracts and the shared precedence rule: [Portable skill contracts](../../docs/MAINTENANCE.md#portable-skill-contracts).
...
6. If `.agents/docs/MAINTENANCE.md` exists, ensure the **Skills docs registry** row for `write-plan` is present and current (docs interaction, durable paths touched, and [`CONTRACT.md`](references/CONTRACT.md) link). Add the row when missing; update it when behavior changes.
