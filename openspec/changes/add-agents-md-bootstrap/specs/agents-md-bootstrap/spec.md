...
Lets the AKSK bootstrap flow give consumer repos a senior-engineer behavioral baseline in their root `AGENTS.md`: seed it from the vendored FerroxLabs/agents-md file when absent, and force an explicit, user-driven decision when one already exists — never overwriting silently.
