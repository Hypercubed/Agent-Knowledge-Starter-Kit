...
- `attach_section.mjs` (as extended by `aksk-bootstrap-system`'s `bootstrap.mjs`) appends marker-delimited AKSK sections (`AKSK:ROUTING`/`AKSK:LIFECYCLE`, preserving `OPENWIKI:START/END`) to root router files — after `aksk-bootstrap-system`, per-repo bootstrap already handles `openspec init`, `.agents` scaffold, contract attachment, and routing-block merge; this change adds no upstream behavioral initializer beyond the baseline seeding (which runs first).
