...
description: How the kit is packaged and validated, the two supported install lanes, caret-pinned versions, rooted .openwikiignore generation rules, and how OpenWiki reaches every agent via the canonical universal store with registry vs npx lane selection and receipt-based ownership.
tags:
- distribution
- agent-spread
...
* `.claude-plugin/plugin.json` lists the 4 user skills (`aksk-bootstrap`, `task-closeout`, `learning-distill`, `docs-lint`) and is loaded by default. Maintainer-only skills marked `metadata.internal: true` (historically `generate-example`) are excluded.
...
`install-lanes` defines exactly two supported lanes, both converging on `aksk-bootstrap` scripts:

| Lane | Entry | What it does |
|------|-------|--------------|
| **Preferred: agent-assisted via `aksk-bootstrap`** | Agent runs `npx skills add -g -a <self-reported> Hypercubed/Agent-Knowledge-Starter-Kit --skill aksk-bootstrap` (or with a local checkout path) then runs that skill; or deterministic `node .agents/skills/aksk-bootstrap/scripts/bootstrap.mjs [repo-root]` with EXECUTE/INSTRUCT fallback | Preflights Node >=22, installs peer tools once per user (`npm i -g`), verifies skills in canonical store, scaffolds per-repo `.agents/` / `openspec` / `openwiki` contract / routing blocks, spreads integrations per lane ladder |
...
`aksk-bootstrap-system` defines exactly one lane per host that should receive an integration. The choice is made by whether the self-reported host is in the registry.
...
The v0.4.3 registry (`HOST_TARGETS` with user/project scopes) owns all install formats for its three hosts. Each `openwiki integrations install <host>` does one atomic transaction:
...
Registry installers oriented at remote MCP servers (`Smithery`, `mcp-get`) and the superseded bare `npx skills add` without `-a` are excluded — they fight receipt/hash verification, transactional rollback, and symlink checks.
...
For the npx lane, ownership is checked via `~/.agents/skills/<name>/SKILL.md` and the host mirror presence; the same skip-or-force discipline applies to the openwiki lifecycle skill. Installing two managers into one destination is prevented by this check plus the symlink/non-directory-parent rejections. The rule is installer-owned — bootstrap never guesses supported-host formats.

## Never Hardcode Agent Skill Paths

Agent skill paths diverge and keep drifting. Bootstrap and spread derive every target from the registry at runtime via `openwiki integrations list`. No path matrix is maintained in kit code. The related instruction is page-specific and normative: derive from `openwiki integrations list` and respect receipt ownership; skip or require `--force` for modified installs. Skill installs always use `npx skills add -g -a <self-reported>` so the Skills CLI itself resolves the universal plus host directories.

## Bundle Source and Update Lifecycle

Two bundle sources, one version source:

* **Registry lane** — canonical bundle is always `<npm-root>/openwiki/integrations/openwiki` from the installed package. The receipt's `version` field pins the installed bundle to a known `openwiki` version (visible via `npm ls openwiki`).
...
Global lane: `npm i -g @fission-ai/openspec@latest openwiki@latest` once per user, skipping any tool already at a compatible version. Versions are read via `versionsFromPackageJson()` from `references/versions.json`. Ordering is verified: `openwiki` must resolve before registering `openwiki mcp --host <target>` in host configs.

Per-repo lane (after global lane): verify skills in the canonical store (`~/.agents/skills/<name>/SKILL.md` plus host mirror, installing via `npx skills add -g -a <self-reported>` if missing), then `openspec init` when missing, minimal `.agents/` scaffold from kit templates, curation-contract attachment to `openwiki/INSTRUCTIONS.md` via append-only `AKSK:WIKI-CONTRACT` markers, and routing-block merge into root `AGENTS.md` preserving any existing `OPENWIKI:START/END` block.
...
sequenceDiagram
    ...
    alt EXECUTE - can run locally
        B->>G: npm i -g @fission-ai/openspec openwiki if missing caret from versions.json
        B->>B: Verify skills ~/.agents/skills plus host mirror npx skills add -g -a self-reported if missing
        B->>R: openspec init scaffold .agents/ attach contract routing block
        B->>S: Detect self-reported host pick lane per host check receipts
...
Reuse boundary: the contract template, skill set, and attach semantics are owned by `adopt-openspec-openwiki`. Bootstrap copies them; variations belong in the source change.
...
* **OpenWiki layer** — `.openwikiignore` is defense in depth beyond git. It contains the anchored `/example/` (slash-anchored, root only) and `.agents/sessions/` with `!.agents/sessions/README.md` re-include. The anchor is load-bearing: `example/.agents/.gitignore` is ignored (`true`), `foo/example/bar.md` and `examples/foo.md` are not (`false`), and nested `example/` folders remain visible. `OpenWikiIgnore.load` validates this.

Treat root `example/` as a generated consumer-install illustration — do not cite files under `example/` as evidence for shipped behavior and do not present its contents as canonical implementation. Treat `.agents/sessions/` (except `README.md`) as temporary closeout bundles — not source truth. Citing `repo://example/.agents/.gitignore` or `repo://.agents/sessions/.../summary.json` fails with `Evidence path is excluded by .openwikiignore` from `dist/evidence/repository/resolver.js`. When a path is added to `.openwikiignore`, walk `openwiki/.claims/**/*.json` and page frontmatter `repo://` sources and retarget to the generator (`repo://.agents/skills/generate-example/run.sh` historically) or canonical file (`repo://.agents/.gitignore`, `sha256:9d3e8002bee34dc78f45dbda50b0668070123c114e1bf70f2f902a61fa049353`). Prose mentions in body are allowed; only `repo://` evidence resources are blocked.
