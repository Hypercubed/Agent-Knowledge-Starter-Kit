...
description: "Captures a finished, blocked, or abandoned task into a five-file temporary session bundle under .agents/sessions/ with canonical task_id identity in summary.json."
...
Deliberately **not** created here: durable `AGENTS.md`, `docs/`, `playbooks/` — those belong to learning-distill initialization or a full kit merge.
...
- **Write scope:** all new files under `.agents/sessions/<session-folder>/`. Never create/edit `.agents/AGENTS.md`, `.agents/docs/**`, `.agents/playbooks/**`, or anything under `.agents/skills/**`. Proposed skill or MAINTENANCE edits go into the bundle as text; learning-distill applies them.
- **Required bundle filenames:** `summary.json`, `active-task.md`, `learning-candidate.md`, `changed-files.txt`, `validation.txt`.
- **Contractual minimum for `summary.json`:** a stable string **`task_id`**. Everything else is optional; the example shows typical keys:
...
- Reference bundle: `.agents/skills/task-closeout/example/task-bundle/` — all five files filled in for a fictional Monaco JSON worker fix.
...
- Planned extension (unshipped): active change `adopt-openspec-openwiki` adds an `openspec_change` field linking the bundle to OpenSpec work and forbids closeout from modifying `openspec/` for in-flight changes ([OpenSpec workflow](../governance/openspec-workflow.md)); `add-task-start` would separately have closeout update an existing `manifest.json` instead of generating metadata.
