...
description: 'Consumers expect `example/.agents/` to mirror a portable install; diffs
  against root `.agents/` usually mean the example tree needs regeneration or a doc
  fix.
...
Expectation that a `diff` between a fresh consumer-style tree (for example `example/.agents/` from [`generate-example`](../../.agents/skills/generate-example/SKILL.md)) and this repository’s root `.agents/` should be empty aside from sessions.
