...
description: 'The generated example tree contains large slices of this repository’s
  durable `.agents/docs/`, playbooks, or other maintainer paths instead of the minimal
...
`example/.agents/` closely matches root `.agents/` (many extra docs, playbooks, or indexes) rather than the **small** tree produced by [`generate-example`](../../.agents/playbooks/generate-example.md).
...
- Remove the fat `example/` tree and regenerate it: `bash .agents/skills/generate-example/run.sh` (see [`.agents/playbooks/generate-example.md`](../../.agents/playbooks/generate-example.md)).
